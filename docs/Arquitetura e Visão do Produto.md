...
-   **Componente Central: Microserviço de Conversão**
    -   Um serviço pequeno e isolado, hospedado em uma plataforma como **Railway** ou **Render**, cuja única responsabilidade é executar o `ffmpeg`.
    -   Este serviço expõe um endpoint de API seguro que pode ser chamado para iniciar um trabalho de conversão.
...
-   **Fluxo de Dados:**
    ...
    3.  **Orquestração:** A Edge Function valida o plano do usuário e, se autorizado, envia uma requisição segura para o microserviço de conversão, informando a URL do vídeo bruto.
    4.  **Processamento:** O microserviço baixa o vídeo, executa os comandos `ffmpeg` para gerar as versões otimizadas para cada plataforma (Reels, Threads, etc.).
