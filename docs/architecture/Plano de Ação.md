...
- **[x] 3. Bloquear E-mails Temporários:** - **Status:** Concluído. - **Notas:** Implementado através de uma Edge Function (`signup-validation`) que busca a lista de bloqueio diretamente do repositório oficial no GitHub. Isso garante que a lista esteja sempre atualizada, ao contrário de pacotes NPM ou APIs de terceiros. A função usa cache para otimizar a performance.
...
## Sessão: Recrutamento de Afiliados (Em Andamento)

Foco em expandir o alcance do PostPulsar através de parcerias com afiliados, utilizando marketplaces e comunidades de marketing digital.

- [ ] **1. Análise de Plataformas de Afiliados:**
  - **BlackHatWorld:** Tentativa de postagem resultou em aviso. A plataforma exige uma conta paga (Jr. VIP ou Marketplace Seller) para anunciar no Marketplace.
  - **Reddit (r/marketing):** Analisado e descartado devido a regras estritas contra auto-promoção.
  - **Indie Hackers:** Analisado e descartado para uma ação imediata, pois requer privilégios especiais que são pagos ou adquiridos com o tempo.
  - **Product Hunt:** Lançamento já realizado anteriormente pelo usuário.

- [ ] **2. Configuração no WarriorPlus:**
  - **Status:** Perfil de vendedor (vendor) criado e aguardando aprovação da plataforma.

- [ ] **3. Configuração no JVZoo (Em Andamento):**
  - **Objetivo:** Cadastrar o PostPulsar como um produto para atrair afiliados da rede JVZoo.
  - **Passos Concluídos:**
    - Definição do produto (Plano Pro, $29) como `Single Payment` com a opção `Allow Repeated Purchases` ativada para garantir comissões recorrentes a cada compra manual.
    - Definição da comissão de afiliados para **30%**.
    - Configuração da aprovação de afiliados como `Manual Approval`.
    - Definição da política de **não reembolso**, utilizando o texto de conformidade recomendado pelo JVZoo.
    - Criação de imagens SVG para o Banner (`162x20`) e Imagem do Produto (`505x217`).
    - Criação da página de "Obrigado" (`/src/pages/thank-you.astro`) para redirecionamento pós-compra.
    - Configuração dos detalhes de suporte, informações do recibo e timers.
  - **Próximo Passo:** Configurar a seção "Compliance" no JVZoo e implementar o webhook (IPN) para criação automática de contas.

## Sessão: Integração com WarriorPlus (Em Andamento)

Foco em integrar o PostPulsar com o marketplace WarriorPlus para processar vendas e automatizar a entrega de acesso.

- [x] **1. Configurar Produto e Oferta no WarriorPlus:**
  - [x] Preenchidas as informações do produto "PostPulsar Pro Plan".
  - [x] Criada a "Offer" (Oferta) para o produto.
  - [x] Configurado cupom de 100% de desconto para permitir uma compra de teste real.
- [x] **2. Criar Página de Vendas Dedicada:**
  - [x] Criada a página `src/pages/sales/warriorplus.astro` para atender aos requisitos de verificação do WarriorPlus.
  - [x] Adicionados o botão de compra e o disclaimer obrigatório do WarriorPlus.
- [x] **3. Corrigir Política de Segurança de Conteúdo (CSP):**
  - [x] Modificado o `vercel.json` para permitir scripts, imagens e frames do `warriorplus.com` e `vercel.live`, resolvendo os erros de verificação da página.
- [x] **4. Implementar Estrutura do Webhook (Postback URL):**
  - [x] Criada a Supabase Edge Function `warriorplus-postback-handler`.
  - [x] Implementada a lógica de verificação de segurança usando a "Security Key" da conta.
- [ ] **5. Submeter Oferta para Aprovação:**
  - [ ] A oferta está pronta para ser enviada para aprovação no painel do WarriorPlus.

### Próximos Passos (WarriorPlus)

- [ ] **1. Finalizar Lógica do Webhook (`warriorplus-postback-handler`):**
  - [ ] Implementar a lógica para mapear o ID do produto comprado a um plano no PostPulsar.
  - [ ] Implementar a criação de um novo usuário (se ele não existir) e o envio do e-mail para criação de senha.
  - [ ] Implementar a atualização do plano para um usuário existente.
- [ ] **2. Realizar Compra de Teste Real:**
  - [ ] Após a aprovação da oferta pelo WarriorPlus, o usuário deve realizar uma compra de teste usando o cupom de 100% de desconto.
  - [ ] Verificar os logs para confirmar que a notificação foi recebida, verificada e processada com sucesso pela lógica finalizada.