...
- **O Problema:** A publicação de carrosséis contendo um ou mais vídeos no Instagram falha consistentemente. A publicação de carrosséis de imagens e de vídeos únicos (como Reels) funciona perfeitamente, mas a combinação das duas funcionalidades resulta em um erro de processamento na API da Meta.
- **A Investigação:**
    1.  **Documentação Contraditória:** A documentação oficial da Meta é ambígua. Uma parte afirma que carrosséis podem conter vídeos, mas outra seção afirma que `Reels` não são permitidos em carrosséis, sugerindo que o `media_type` para vídeos em carrossel deveria ser `VIDEO`.
    2.  **Teste com `media_type: 'VIDEO'`:** Ao seguir a documentação e usar `VIDEO`, a API passou a retornar um erro `ERROR` imediato, em vez do `timeout` que ocorria antes. Isso indicou uma falha clara, mas sem um subcódigo de erro que explicasse a causa.
    3.  **Teste com `media_type: 'REELS'`:** Revertemos a lógica para usar `REELS` para todos os vídeos, incluindo os de carrossel. Isso fez com que a API parasse de dar o erro imediato e voltasse a tentar processar o vídeo, mas a operação ainda falhava mais tarde.
    4.  **Limite de Duração:** Uma pesquisa em documentações de ferramentas de terceiros (Metricool) revelou uma possível limitação não documentada oficialmente: vídeos em carrosséis não poderiam exceder 60 segundos. No entanto, testes com vídeos curtos (5 segundos) também falharam, descartando a duração como a causa principal do problema.
- **Conclusão (Estado Atual):** A API de publicação de carrosséis de vídeo do Instagram é **extremamente instável e não confiável no momento**. Apesar de a documentação sugerir que é possível, os testes práticos mostram que a combinação de parâmetros necessária para o sucesso é desconhecida ou a funcionalidade está simplesmente quebrada no lado da Meta. A publicação de vídeo único como Reel e de carrossel de imagens continua funcionando de forma estável.
- **Lição:** Para funcionalidades complexas em APIs de terceiros que se mostram instáveis, é crucial isolar o problema e não presumir que a documentação está 100% correta ou completa. Testes empíricos com diferentes combinações de parâmetros são necessários. Neste caso, a lição é reconhecer quando uma API não é confiável e evitar implementar a funcionalidade para não impactar negativamente a experiência do usuário. A funcionalidade de carrossel de vídeo no Instagram deve ser considerada **desativada** até que a Meta forneça uma documentação mais clara ou corrija a instabilidade da API.