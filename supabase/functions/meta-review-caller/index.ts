...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Attempting to call Threads Profile Discovery API for username: ${usernameToLookup}`,
    );

    // Make the call to the correct Threads API endpoint
    const response = await fetch(
      `https://graph.threads.net/v1.0/profile_lookup?username=${usernameToLookup}&access_token=${accessToken}`,
    );
