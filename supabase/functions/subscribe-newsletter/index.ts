...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[LOG] Received email:', email, 'and client_user_id:', client_user_id); // Log 2: Received data
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return new Response(JSON.stringify({ status: 'error', error: 'Invalid email format.' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 400,
      });
    }
    ...
    if (error) {
      ...
      return new Response(JSON.stringify({ status: 'error', error: 'Failed to subscribe.' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 500,
      });
    }
    ...
    console.log('[LOG] Upsert successful. Subscriber ID:', subscriberId, 'Token:', confirmationToken); // Log 3: Upsert successful
    ...
    console.log('[LOG] Attempting to insert into email_queue with payload:', emailPayload); // Log 4: Attempting insertion
    ...
    if (emailQueueError) {
      console.error('[ERROR] Error inserting message into email_queue:', emailQueueError); // Log 5: Email queue insertion error
