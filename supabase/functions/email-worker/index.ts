...
if (!RESEND_API_KEY || !SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.error('Missing one or more required environment variables: RESEND_API_KEY, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY');
  Deno.exit(1);
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (fetchError) {
      ...
      return new Response(JSON.stringify({ status: 'error', error: 'Failed to fetch tasks.' }), { status: 500 });
    }

    if (!tasks || tasks.length === 0) {
      console.log('No pending emails to process.');
      return new Response(JSON.stringify({ status: 'success', message: 'No messages to process.' }), { status: 200 });
    }
    ...
    <tr>
      <td align="center">
        <div style="max-width: 600px; margin: 0 auto; text-align: center;">
          ...
          console.log(`Resend API success response for ${email} (taskId: ${taskId}):`, emailData);
          await supabaseClient
            .from('email_queue')
            .update({ status: 'sent', sent_at: new Date().toISOString() })
            .eq('id', taskId);
          console.log(`Confirmation email sent to ${email} (taskId: ${taskId}).`);

        } else {
          console.warn(`Unknown task type: ${payload.type} (taskId: ${taskId}). Marking as failed.`);
          await supabaseClient
            .from('email_queue')
        ...
        console.error(`Failed to process task ${taskId}:`, processError.message);
        ...
        console.log(`Task ${taskId} marked as '${newStatus}' after ${newRetryCount} retries.`);
      }
    }

    return new Response(JSON.stringify({ status: 'success', message: `Processed ${tasks.length} tasks.` }), { status: 200 });
  } catch (error) {
