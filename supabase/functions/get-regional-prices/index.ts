...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[get-regional-prices] Detected IP: ${clientIp}, Country: ${countryCode}`,
    );

    const targetCurrency =
      (countryCode && countryToCurrency[countryCode]) || null;
    const applyDiscount =
      countryCode && discountTierCountries.includes(countryCode);
    console.log(
      `[get-regional-prices] Target Currency: ${targetCurrency}, Apply Discount: ${applyDiscount}`,
    );
    ...
    const pricePromises = products.map((product) => {
      ...
      console.log(
        `[get-regional-prices] For product ${product.id}, found Price ID: ${priceId}`,
      );

      if (priceId) {
        return stripe.prices
          .retrieve(priceId)
          .then((stripePrice) => ({ productId: product.id, stripePrice }));
      }
    ...
    for (const { productId, stripePrice } of results) {
      if (stripePrice) {
        ...
        if (applyDiscount && priceData.currency === "usd") {
          ...
          console.log(
            `[get-regional-prices] Applied 50% discount to ${productId}. New amount: ${priceData.amount}`,
          );
        }
