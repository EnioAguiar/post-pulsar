...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (referrerError) {
      console.error("Error querying for referrer profile:", JSON.stringify(referrerError, null, 2));
    }

    if (referrerError || !referrerProfile) {
      console.error(`Referrer not found for code: ${referral_code}. Profile data was:`, referrerProfile);
      return new Response(JSON.stringify({ message: "Processed, but no referrer found." }), {
    ...
    console.log(`Attempting to insert referral: ${referrerId} -> ${referredId}`);
    ...
    if (insertError && insertError.code !== '23505') {
      console.error("Error inserting referral:", JSON.stringify(insertError, null, 2));
      throw insertError;
    }
