...
serve(async (req) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Twitter request token failed with status ${response.status}: ${errorBody}`,
      );
    }
    ...
    if (!requestToken.key || !requestToken.secret) {
      throw new Error(
        "oauth_token or oauth_token_secret not found in Twitter response",
      );
    }
    ...
    if (stateError) {
      console.error(
        "[twitter-auth-start] Error saving state to Supabase:",
        stateError,
      );
