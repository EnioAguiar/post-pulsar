...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!profileResponse.ok) {
      ...
      throw new Error(profileData.error?.message || "Failed to fetch Threads profile.");
    }
    ...
    if (!threadsUserId || !threadsUsername) {
      console.error("Could not extract id or username from profile data:", profileData);
      throw new Error("Failed to get Threads user details.");
    }
    console.log(`Successfully fetched Threads User ID: ${threadsUserId} and Username: ${threadsUsername}`);
    ...
    console.error('Error in threads-auth-callback:', error.message)
