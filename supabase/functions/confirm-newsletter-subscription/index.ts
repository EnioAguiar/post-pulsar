  ...
  try {
    ...
    if (!token || !email) {
      return Response.redirect(
        `${SITE_URL}/newsletter/error?message=missing_params`,
        302,
      );
    }
    ...
    if (fetchError || !subscriber) {
      console.error(
        "Error fetching subscriber or subscriber not found:",
        fetchError,
      );
      return Response.redirect(
        `${SITE_URL}/newsletter/error?message=invalid_token_or_email`,
        302,
      );
    }
    ...
    if (updateError) {
      ...
      return Response.redirect(
        `${SITE_URL}/newsletter/error?message=update_failed`,
        302,
      );
    }
    ...
    return Response.redirect(
      `${SITE_URL}/newsletter/error?message=internal_error`,
      302,
    );
  }
});
