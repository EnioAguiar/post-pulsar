...
serve(async (req) => {
  ...
  try {
    ...
    if (userError) {
      ...
      return new Response(JSON.stringify({ status: 'error', error: 'Authentication failed' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200,
      });
    }
    ...
    if (!provider || !credentials) {
      return new Response(JSON.stringify({ status: 'error', error: 'Missing provider or credentials' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200,
      });
    }
    ...
    if (provider === 'telegram') {
      if (!credentials.bot_token || !credentials.channel_id) {
        return new Response(JSON.stringify({ status: 'error', error: 'Missing Telegram Bot Token or Channel ID' }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 200,
        });
      }
      ...
      if (!credentials.webhook_url) {
        return new Response(JSON.stringify({ status: 'error', error: 'Missing Discord Webhook URL' }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 200,
        });
      }
      ...
      return new Response(JSON.stringify({ status: 'error', error: 'Unsupported provider' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200,
      });
    }
    ...
    if (upsertError) {
      ...
      return new Response(JSON.stringify({ status: 'error', error: 'Failed to save connection to database.' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200,
      });
    }
