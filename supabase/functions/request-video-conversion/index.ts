...
async function handler(req: Request) {
  console.log(
    `INFO: [${new Date().toISOString()}] Received request: ${req.method} ${req.url}`,
  );
  ...
  try {
    ...
    if (profile.plan_type.replace(/'/g, "") !== "pro") {
      console.warn(
        `WARN: User ${userId} with plan '${profile.plan_type}' attempted to access Pro feature.`,
      );
      return new Response(
        JSON.stringify({
          error: "Forbidden: Video conversion is a Pro feature.",
        }),
        {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!videoUrl || !outputFileName) {
      console.error(
        "ERROR: Missing 'videoUrl' or 'outputFileName' in request body.",
      );
      return new Response(
        JSON.stringify({ error: "Missing videoUrl or outputFileName" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (!analyzeResponse.ok) {
      ...
      console.log(
        "DEBUG: Analysis successful. Data:",
      ...
      const hasAudio = analysisData.streams.some(
      ...
      console.log(
        `DEBUG: Compliance check - Bitrate OK: ${isBitrateOk} (${(bitrate / 1000000).toFixed(2)} Mbps)`,
      );
      ...
      console.log(
        `DEBUG: Compliance check - Framerate OK: ${isFramerateOk} (${frameRate.toFixed(2)} FPS)`,
      );
      ...
      if (isCompliant) {
        console.log(
          "INFO: Video is compliant. Calling /clean to remux and prevent metadata issues.",
        );
        ...
        if (!cleanResponse.ok) {
          ...
          throw new Error(
            `Clean service failed with status ${cleanResponse.status}: ${errorBody}`,
          );
        }

        const cleanData = await cleanResponse.json();
        console.log(
          "SUCCESS: Video cleaned successfully. Returning response to client.",
        );
    ...
    console.log(
      `DEBUG: Converter service (/convert) responded with status: ${convertResponse.status}`,
    );
    if (!convertResponse.ok) {
      const errorBody = await convertResponse.text();
      console.error("ERROR: Converter service (/convert) failed:", errorBody);
      throw new Error(
        `Converter service failed with status ${convertResponse.status}: ${errorBody}`,
      );
    }

    const responseData = await convertResponse.json();
    console.log(
      "SUCCESS: Video conversion successful. Returning response to client.",
    );
    ...
    console.error(
      "FATAL: Unhandled exception in function:",
      errorMessage,
      stack,
    );
