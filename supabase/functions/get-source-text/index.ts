...
serve(async (req) => {
  ...
  try {
    ...
    if (!serviceKey) {
      console.error("[GET_SOURCE_TEXT_LOG] CRITICAL: SUPABASE_SERVICE_ROLE_KEY is not set.");
    ...
    if (isMedia) {
      console.log(`[GET_SOURCE_TEXT_LOG] Detected media URL: ${url}. Calling video-converter-service for transcription.`);
      ...
      const transcribeResponse = await fetch(`${converterUrl}/transcribe`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${CONVERTER_SERVICE_API_KEY}` },
      ...
      if (transcribeResult.status === "success" && transcribeResult.text) {
        ...
        console.log(`[GET_SOURCE_TEXT_LOG] Transcription successful. Text length: ${cleanedText.length}`);
      } else {
        throw new Error(`Transcription service returned an error: ${transcribeResult.error || "Unknown error"}`);
      }
    } else { // Regular URL scraping
      ...
      console.log(`[GET_SOURCE_TEXT_LOG] Scrape complete. Body length: ${cleanedText.length}`);
    }

    // --- 5. CHARGE PULSES ---
    console.log(`[GET_SOURCE_TEXT_LOG] Calling RPC to charge ${pulseCost} pulses for source extraction...`);
    ...
    if (rpcError) {
      console.error("[GET_SOURCE_TEXT_LOG] RPC error charging pulse:", rpcError.message);
      throw new Error("Failed to charge pulse for source extraction.");
    }
    console.log(`[GET_SOURCE_TEXT_LOG] RPC success. ${pulseCost} pulses charged for user: ${user.id}`);
    ...
    console.error("[GET_SOURCE_TEXT_LOG] Error in function handler:", errorMessage);
    ...
    if (errorMessage.includes("429 Too Many Requests") || errorMessage.includes("QuotaFailure") || errorMessage.includes("503")) {
      ...
      return new Response(JSON.stringify({ status: "error", error: userMessage, errorCode: "AI_RATE_LIMIT_EXCEEDED" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      });
    }
