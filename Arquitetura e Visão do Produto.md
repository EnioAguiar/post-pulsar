...
A conexão com redes sociais (começando pelo LinkedIn) é implementada através de um fluxo OAuth 2.0 seguro, orquestrado por um conjunto de Edge Functions.

1.  **Início do Fluxo (Frontend):** Na página de conexões, o usuário clica em "Lincar Conta". O frontend chama a função `linkedin-auth-start`, enviando o ID do usuário logado.
2.  **Geração da URL de Autorização (`linkedin-auth-start`):** Esta função cria uma URL de autorização do LinkedIn. Crucialmente, ela anexa um parâmetro `state` que contém o `user_id` (codificado em Base64) para identificar o usuário de forma segura ao longo do processo. Ela utiliza os escopos do padrão OpenID Connect (`openid`, `profile`, `email`) e o escopo de postagem (`w_member_social`).
3.  **Callback e Troca de Token (`linkedin-auth-callback`):** Após o usuário autorizar no LinkedIn, ele é redirecionado para esta função. A função:
    - Valida o `state` para recuperar o `user_id`.
    - Troca o `code` de autorização recebido por um `access_token` válido.
    - Salva o token, escopos e data de expiração na tabela `social_connections` do banco de dados, associando ao `user_id` correto.
    - Redireciona o usuário de volta para a página de conexões no frontend.
4.  **Ação de Postar (`publish-to-social`):** Ao clicar em "Postar", esta outra Edge Function busca o `access_token` armazenado para o usuário, e o utiliza para se comunicar com a API da rede social e publicar o conteúdo. Isso garante que o token nunca seja exposto no lado do cliente.
