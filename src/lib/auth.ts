...
export function manageAuth() {
  ...
  supabase.auth.onAuthStateChange((event, session) => {
    ...
    if (navLinks) {
      ...
      if (user) {
        loggedInLinks.forEach(link => (link as HTMLElement).style.display = 'inline-block');
        loggedOutLinks.forEach(link => (link as HTMLElement).style.display = 'none');
      } else {
        loggedInLinks.forEach(link => (link as HTMLElement).style.display = 'none');
        loggedOutLinks.forEach(link => (link as HTMLElement).style.display = 'inline-block');
      }
